...
class SurrogateModel:
    ...
    def prepare_dataframe(df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_copy = df.copy()
        if "glazing_material" not in df_copy.columns:
            df_copy["glazing_material"] = "glazing_low_e"
        if "humidity_pct" not in df_copy.columns:
            df_copy["humidity_pct"] = 30.0
        if "diurnal_amplitude_c" not in df_copy.columns:
            if "ambient_max_temp" in df_copy.columns and "ambient_min_temp" in df_copy.columns:
                df_copy["diurnal_amplitude_c"] = df_copy["ambient_max_temp"] - df_copy["ambient_min_temp"]
            else:
                df_copy["diurnal_amplitude_c"] = 10.0
        if "is_greenhouse" not in df_copy.columns:
            df_copy["is_greenhouse"] = 0
        if "is_ground_coupled" not in df_copy.columns:
            df_copy["is_ground_coupled"] = df_copy["shape"].apply(lambda s: 1 if s in ("bunker_bermed", "igloo_catenary") else 0)

        clean_df = df_copy[FEATURE_COLS].copy()
        wall_map = {
    ...
    def extract_features_single(params: SimulationParams, climate: ClimateInput) -> pd.DataFrame:
        ...
        is_pcm = 1 if "pcm" in str(params.wall_material).lower() else 0
        is_greenhouse = 1 if getattr(params, "greenhouse_mode", False) else 0
        is_ground_coupled = 1 if params.shape in ("bunker_bermed", "igloo_catenary") else 0
        glazing_mat = getattr(params, "glazing_material", "glazing_low_e") or "glazing_low_e"
        humidity = getattr(climate, "humidity_pct", 30.0) or 30.0
        diurnal_amp = float(max(temps) - min(temps))
    ...
    def train(
        ...
        """
        Trains XGBoost surrogate models across all targets on the simulation dataset.
        ...
        for tgt in TARGET_COLS:
            # Per-target tuned hyper-parameters for optimal accuracy across non-linear targets
            if tgt in ["solar_gain_wm2", "u_value", "heat_loss_wm2", "comfort_percent"]:
                n_est = 800
                max_depth = 8
                lr = 0.03
                min_child_weight = 1
                ...
                subsample = 0.90
                colsample = 0.90
            elif tgt in ["max_indoor_temp", "diurnal_swing_c", "min_indoor_temp"]:
                n_est = 600
                max_depth = 7
                lr = 0.03
                min_child_weight = 2
                ...
                subsample = 0.90
                colsample = 0.90
            elif tgt in ["total_weight_kg", "total_cost_inr", "carbon_footprint_kgco2"]:
                n_est = 400
                max_depth = 6
                lr = 0.05
                min_child_weight = 2
