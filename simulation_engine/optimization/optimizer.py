...
def _select_diverse_candidates(
    ...
    print(f"[DIAGNOSTIC STEP 6][Optimizer Selected {len(final_candidates)} Diverse Pareto Candidates]:", flush=True)
...
def optimize(
    ...
    *,
    ...
    print(f"\n[DIAGNOSTIC STEP 6][Optimizer NSGA-II Ingress]:\n"
          f"  Climate: lat={norm_climate.lat}, lon={norm_climate.lon}, alt={norm_climate.altitude}\n"
          f"  Requirements: {reqs}\n"
          f"  NSGA-II Setup: pop_size={pop_size}, generations={generations}, seed={seed}", flush=True)
    ...
    print(f"[DIAGNOSTIC STEP 6][NSGA-II Execution Completed]:\n"
          f"  Generations Evaluated: {generations}, Total Evaluations: {pop_size * generations}\n"
          f"  Strictly Feasible Pareto Count: {len(res.opt) if res.opt is not None else 0}\n"
          f"  Population Size Evaluated: {len(res.pop) if res.pop is not None else 0}", flush=True)
