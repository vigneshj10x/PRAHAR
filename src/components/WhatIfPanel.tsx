 ...
 *
 * PROTOTYPE: live results computed via the same interpolation path in
 * getScenarioResults() — not a real physics engine.
 */
...
import { getScenarioResults } from '../lib/simulationEngine'
...
const WhatIfPanel: FC = () => {
  ...
  useEffect(() => {
    ...
    if (resultsStatus === 'ready' && storedIndoorTemp !== 0) {
      ...
      const A = scenarios.A_baseline.results
      snap = {
        indoorTemp:    A.indoorTemp,
        solarGain:     A.solarGain,
        heatLoss:      A.heatLoss,
        comfortHours:  A.comfortHours,
        heatingDemand: A.heatingDemand,
        estimated:     false,
    ...
    setLiveResult(getScenarioResults(initParams))
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])   // deliberately run only once on mount

  /* ── Slider change handlers ────────────────────────────────────── */
  function handleInsulationChange(val: number) {
    setInsulation(val)
    ...
    setLiveResult(getScenarioResults(params))
  }

  function handleOrientationChange(val: number) {
    setOrientation(val)
    const params: DesignParams = {
      shape:        designParams.shape,
      orientation:  val,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span style={{
              ...
              {orientation}° <span style={{ opacity: 0.7, fontSize: 8 }}>({compassDir(orientation)})</span>
            </span>
          </div>
          <input
            type="range"
